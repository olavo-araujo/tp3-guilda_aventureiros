...
import infnet.tp2_springboot.dto.MissaoResumoDTO;
import infnet.tp2_springboot.dto.RelatorioMissaoDTO;
...
public interface MissaoRepository extends JpaRepository<Missao, Long> {
    @Query("SELECT new infnet.tp2_springboot.dto.MissaoResumoDTO(m.id, m.titulo, m.nivelPerigo, m.status) " +
            "FROM Missao m WHERE " +
    ...
    @Query("SELECT new infnet.tp2_springboot.dto.RelatorioMissaoDTO(m.titulo, m.status, m.nivelPerigo, COUNT(p), SUM(p.recompensaOuro)) " +
